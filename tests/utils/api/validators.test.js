...
test('Test min length ok', () => {
    ...
    expect(minLength('', 5)).toStrictEqual([true, 'Min 5 characters'])
    expect(minLength(null, 5)).toStrictEqual([true, 'Min 5 characters'])
})
...
test('Test max length ok', () => {
    ...
    expect(maxLength('', 5)).toStrictEqual([true, 'Max 5 characters'])
})
...
test('Test same as fail', () => {
    ...
    expect(sameAs('', 'foo')).toStrictEqual([false, 'Value mismatch'])
})

test('Test same as fail override default message', () => {
    expect(sameAs('test', 'foo', 'Value strongly mismatch')).toStrictEqual([
        false,
        'Value strongly mismatch'
    ])
})
...
test('Test many check all ok', () => {
    expect(checks([validateEmail('test@example.com'), sameAs('test', 'test')])).toBe(null)
})

test('Test many check some fail', () => {
    expect(checks([validateEmail('test@example.com'), sameAs('test', 'bar')])).toBe(
        'Value mismatch'
    )
})
